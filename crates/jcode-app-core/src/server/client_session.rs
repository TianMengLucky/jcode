...
pub(super) async fn handle_subscribe(
    ...
    // The anchored working dir rides along so a remote client (SSH TUI) can
    // display the project dir and seed its directory browser — the wire is
    // its only source since no local session file exists.
    let anchor_dir = agent
        .try_lock()
        .ok()
        .and_then(|guard| guard.working_dir().map(str::to_string))
        .or_else(|| {
            swarm_members.try_read().ok().and_then(|members| {
                members
                    .get(client_session_id)
...
pub(super) async fn handle_resume_session(
    ...
    if let Some(live_target_agent) = live_target_agent.as_ref() {
        ...
        *client_session_id = session_id.clone();
        ...
        let anchor_dir = live_target_agent
            .try_lock()
            .ok()
            ...
            .or_else(|| working_dir_override.map(str::to_string));
        // The client anchors its status bar to the session's working dir;
        // without this it keeps the temporary session's `~` after resume.
        let _ = client_event_tx.send(ServerEvent::SessionId {
            session_id: session_id.clone(),
            working_dir: anchor_dir.clone(),
        });
        let _ = client_event_tx.send(ServerEvent::Done { id });
        let mcp_working_dir = anchor_dir.map(PathBuf::from);
        registry
            .register_mcp_tools_for_dir(
                Some(client_event_tx.clone()),
                Some(Arc::clone(mcp_pool)),
    ...
    match result {
        Ok(_prev_status) => {
            ...
            *client_session_id = session_id.clone();
            ...
            .await?;
            // Same wire gap as the live-attach path above: the client needs
            // the resumed session's anchor to update its status bar.
            let anchor_dir = {
                let agent_guard = agent.lock().await;
                agent_guard.working_dir().map(str::to_string)
            };
