...
-> Option<jcode_base::provider_catalog::ResolvedOpenAiCompatibleProfile> {
    ...
    let mut matches = openai_compatible_profiles()
        ...
        .filter_map(|profile| {
            ...
            if jcode_base::provider_catalog::openai_compatible_profile_is_configured(*profile) {
                Some(resolved)
            } else {
                None
            }
...
async fn fetch_models_from_api(
    ...
    let mut models = parse_openai_compatible_models_response(&raw_body).with_context(|| {
            format!(
                ...
                url,
                auth.label(),
                jcode_base::util::truncate_str(&raw_body.trim().replace('\n', "\\n"), 1200)
            )
        })?;
...
pub fn reset_profile_catalog_refresh_tracker_for_tests() {
...
impl WireApi {
    ...
    /// `openai-responses` for the Responses API; anything else (including
    /// unset) keeps the chat/completions default.
    fn from_config(api: Option<&str>) -> Self {
        match api.map(str::trim).filter(|v| !v.is_empty()) {
            Some(v) if v.eq_ignore_ascii_case("openai-responses") => Self::Responses,
            _ => Self::Chat,
        }
    }
}
...
pub struct OpenRouterProvider {
    ...
    /// Set via `api = "openai-responses"` on a `[providers.<name>]` profile.
    wire_api: WireApi,
...
impl OpenRouterProvider {
    ...
    fn configured_max_tokens(_profile_id: Option<&str>) -> Option<u32> {
        if let Ok(raw) = std::env::var("JCODE_OPENROUTER_MAX_TOKENS") {
            ...
            match trimmed.parse::<u32>() {
                ...
                Err(_) => jcode_base::logging::warn(&format!(
    ...
    pub fn explicit_provider_pin_for_current_model(&self) -> Option<String> {
        if !self.supports_provider_features {
            return None;
        }
