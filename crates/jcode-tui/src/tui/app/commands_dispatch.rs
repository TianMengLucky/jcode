...
pub(super) fn ssh_unsupported_command(input: &str) -> bool {
    ...
    matches!(
        ...
        // `/login` has a dedicated SSH flow; the rest have no sideband op yet.
        "/logout"
            | "/auth"
            | "/account"
            | "/accounts"
            ...
            // session store; no sideband op exists yet. (`/save`, `/unsave`,
...
pub(super) fn dispatch_ssh_local_command(app: &mut App, trimmed: &str) -> bool {
    ...
    // is an umbrella handler; gating the command names first keeps
    // non-config sub-handlers unreachable. `/config`, `/agents` and
    // `/swarm-prompt` are safe here because every write now routes through
    // the remote config override + `write_config`/`read_file`/`write_file`
    // sideband ops instead of the laptop's files.
    ...
    super::commands::handle_keys_command(app, trimmed)
        ...
        || super::commands::handle_ssh_prompt_command(app, trimmed)
...
mod tests {
    ...
    fn ssh_ui_blocks_local_actions_and_preserves_text_and_cancel() {
        ...
        app.is_remote = true;
        for command in ["/logout", "/save test", "/git"] {
        ...
        // These are claimed by client-local handlers (config umbrella, debug
        // tooling), so they produce their own output instead of an SSH-mode
        // banner.
        for command in ["/config init", "/reload"] {
            assert!(
                super::dispatch_local_command(&mut app, command),
                "{command}"
            );
        }
        ...
        // `list_sessions` op is queued for the remote poll loop.
    ...
    /// Commands the SSH client may run itself: prompt-shaping commands that
    /// only queue a wire-bound prompt, plus client-local tooling.
    #[test]
    fn ssh_local_dispatch_claims_prompt_and_client_tool_commands() {
        if super::ssh_test_runs_in_child(
            "ssh_local_dispatch_claims_prompt_and_client_tool_commands",
        ) {
            return;
        }
        ...
        for command in [
            ...
            assert!(
                super::dispatch_ssh_local_command(&mut app, command),
                "{command}"
            );
        }
        for command in ["/permissions", "/selfdev", "/transcript", "/usage"] {
            assert!(
                !super::dispatch_ssh_local_command(&mut app, command),
                "{command}"
            );
