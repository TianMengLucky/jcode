...
impl RemoteConnection {
    ...
    /// (SSH remotes, picker rows for non-attached sessions). Named `_by_id`
    /// to stay distinct from `Request::SetSessionSaved`, which addresses the
    /// attached session.
    pub async fn set_session_saved_by_id(
        &mut self,
        session_id: &str,
        saved: bool,
