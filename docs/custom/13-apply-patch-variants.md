...
- Neither tag → `apply_patch` is not registered. Either tag →
  `edit`/`write` are removed from the session tool map (and restored
  when the tags go away, e.g. on model switch). Since v0.88.0 upstream
  merged `multiedit` into `edit` and made `patch` a `resolve_tool_name`
  alias of `apply_patch`, so the exclusion list shrank to the two real
  registry keys. Both tags → configuration error surfaced to the user;
  neither variant is enabled and the editing family stays.
